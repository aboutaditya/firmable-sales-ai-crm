  ...
  return (
    <Modal eyebrow="ADMIN CONSOLE" title="Set thresholds & assign accounts" subtitle="Manage sales reps while you work." onClose={onClose}>
      ...
      {selectedUser && <div className="admin-user-detail">
        <label className="admin-field">Role<select value={selectedUser.role} onChange={event => onUpdateAdminUser({ ...selectedUser, role: event.target.value })}><option value="admin">Admin</option><option value="sales_manager">Sales Manager</option><option value="sales_rep">Sales Rep</option></select></label>
        ...
        <button onClick={() => onSaveAdminUser(selectedUser)} disabled={adminSaving || !authenticated}>{adminSaving ? "Saving…" : "Save settings"}</button>
