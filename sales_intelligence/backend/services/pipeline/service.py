...
from sales_intelligence.backend.services.dataset_runs import DatasetRunService
from sales_intelligence.backend.services.pipeline.checkpoint import CheckpointStore
from sales_intelligence.backend.services.pipeline.manifest import RunManifest
from sales_intelligence.backend.services.pipeline_runs import PipelineRunTracker
        ...
        *,
        ...
        try:
            dataset_run_service.start(
                ...
                score_version=SCORE_VERSION, current_record=resumed_count,
                company_count=len(initial_profiles),
            )
            run_id = run_tracker.start(run_type="etl", dataset_version=dataset_version, score_version=SCORE_VERSION, source=str(source))
        ...
        try:
            ...
            profiles = aggregate(records, initial_profiles=initial_profiles, on_record=save_checkpoint)
            ...
            result_manifest = {
                ...
                "score_version": SCORE_VERSION, "source": str(source),
